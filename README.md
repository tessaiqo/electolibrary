...
### 5.6. Фильтрация и сортировка

В `BooksView`:
- **Фильтр по статусу**: «Все» / «В наличии» / «Нет в наличии».
- **Сортировка**:
  - по дате добавления (новые / старые),
  - по алфавиту названия (А→Я, Я→А, с `localeCompare(_, 'ru')` для корректной русской сортировки).

Реализовано через `computed.filteredBooks` — изменение `filterStatus` или
`sortKey` мгновенно перерисовывает список без обращения к серверу.

Условный рендеринг пустого списка реализован через `v-if="books.length === 0"`
в компоненте `BookList`.
